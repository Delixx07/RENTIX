...
- **Autentikasi + E-KYC** — Register/login nyata via Supabase Auth; upload KTP ke Storage, status verifikasi tersimpan per pengguna
- **Guard E-KYC** — Checkout hanya bisa setelah identitas terverifikasi
- **Rating Dua Arah** — Penyewa menilai pemilik di halaman produk, pemilik menilai penyewa dari Dashboard
- **Kelola Produk** — Pemilik melihat & menghapus produknya sendiri, upload foto nyata ke Storage
- **Wishlist & Keranjang** — Tersimpan otomatis di browser (tetap ada setelah refresh)
...
- **Dashboard** — Riwayat sewa, produk saya, verifikasi E-KYC & KTM, statistik akun
...
3. Buat query baru lagi, salin seluruh isi [`supabase/migrations.sql`](supabase/migrations.sql), tempel, lalu **Run**.

   > Ini menambahkan: trigger auto-update rating produk, bucket Storage (`kyc` privat & `products` publik) untuk upload KTP/foto, serta kebijakan rating dua arah. Aman dijalankan ulang.
...
├── src/
│   ├── components/         # Navbar, Footer, ProductCard, Icon, Stars,
│   │                      # ProductSkeleton, Toast, Modal
│   ├── pages/             # Home, Browse, Detail, Cart, Wishlist, ListItem,
│   │                      # HowItWorks, Login, Register, Trust, Promo, Dashboard
│   ├── lib/               # supabase.js (client) & api.js (data layer + fallback)
│   ├── data/              # Katalog statis (fallback) & helper
│   ├── store/             # Zustand store (auth + cart + wishlist + UI, persist)
│   ├── App.jsx            # Routing utama
│   ├── main.jsx           # Entry point
│   └── index.css          # Style global & variabel warna
├── supabase/
│   ├── schema.sql         # Skema database + RLS + seed data
│   └── migrations.sql     # Trigger rating, Storage buckets, rating dua arah
